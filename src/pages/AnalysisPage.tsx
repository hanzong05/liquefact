...
function dbRecordsToDatasetBoreholes(records: DbBoreholeRecord[]): DatasetBorehole[] {
  const map = new Map<string, DatasetBorehole>();
  for (const r of records) {
    ...
    map.get(key)!.depthRows.push({
      depthOfSoil: r.DepthOfSoil,
      totalUnitWeight: r['Unit Weight (γ)'],
      n60: r['Corrected SPT-N Value (N1(60))'],
      finesContent: r['Fines Content'],
      peakGroundAcceleration: r['Peak Ground Acceleration'],
      groundWaterLevel: r['Groundwater Level (m)'],
      soilType: r.SoilType,
      modulusOfElasticity: r['Elastic Modulus (Es) (MN/m²)'],
    });
  }
...
export function AnalysisPage() {
  ...
  const [apiBoreholes, setApiBoreholes] = useState<DatasetBorehole[]>([]);
  ...
  useEffect(() => {
    ...
    getBoreholes(ac.signal)
      .then((records) => setApiBoreholes(dbRecordsToDatasetBoreholes(records)))
      .catch(() => {});
    return () => ac.abort();
  }, []);
    ...
    (async () => {
      ...
      try {
        ...
        await liq.computeLiquefaction({
          ...
          boreholes: apiBoreholes.length > 0 ? apiBoreholes : undefined,
        });
  ...
  const activeBoreholeSrc = apiBoreholes.length > 0 ? apiBoreholes : datasetBoreholes;

  const nearestBorehole = useMemo(() => {
    let nearest: {id: string; distanceKm: number} | null = null;
    for (const borehole of activeBoreholeSrc) {
      if (
    ...
    return nearest;
  }, [mapLat, mapLng, activeBoreholeSrc]);
  ...
  const nearestInterpEligibleKmFromAnalysis = useMemo(
    ...
    [analysisSiteLat, analysisSiteLng, activeBoreholeSrc],
  );

  // No data only when the dataset has no boreholes at all — runModel always
  // uses the single nearest borehole regardless of distance.
  ...
  const neighborCalibration = useMemo(() => {
    ...
    return calibrateLpiFromNeighbors(
      ...
      {blendWeight: 0, proximityHighRiskKm: 0},
    );
  }, [
    analysis,
    analysisSiteLat,
  ...
  const applyHeaderGo = async () => {
    let la = Number.parseFloat(headerLat);
              ...
              {noBoreholeDataWithinNeighborRadius ? (
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-amber-950/95">
                    Warning: No borehole data available in the dataset.
                  </p>
                </div>
              ) : (
                <>
